...
cfct.hide = function() {
	return '<a class="hide_toggle">Hide</a>';
}
...
cfct.ajax_post_comments = function() {
	jQuery('p.comments-link a').unbind().click(function() {
		...
		jQuery('#post-comments-' + post_id + '-target').html(cfct.loading()).load(CFCT_URL + '/index.php?cfct_action=post_comments&id=' + post_id).append(cfct.hide());
		return false;
	});
}
